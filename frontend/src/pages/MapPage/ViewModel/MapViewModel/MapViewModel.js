...
const useMapViewModel = ({ mapData, busLines, stops, selectedStopCoordinates, mode, path, onMapClick }) => {
    ...
    // FIXED: Coordinate formatting for MapBox API calls
    useEffect(() => {
        const map = mapRef.current;
        if (!map) return;
            ...
            if (Array.isArray(path) && path.length > 0) {
                const processSegments = async () => {
                    ...
                    for (let i = 0; i < path.length; i++) {
                        ...
                        if (type === 'bus' && to) {
                            // Bus stops - assuming pointX is lng, pointY is lat (correct format)
                            waypoints = to
                                .map(stop => `${stop.pointX},${stop.pointY}`)
                                .join(';');
                        } else if (coords && coords.length >= 2) {
                            // FIXED: Keep coordinates in lng,lat format for MapBox API
                            waypoints = coords
                                .map(coord => Array.isArray(coord)
                                    ? `${coord[0]},${coord[1]}`  // FIXED: Keep as lng,lat (not lat,lng)
                        ...
                        if (waypoints) {
                            ...
                            try {
                                ...
                                if (data.routes && data.routes.length > 0) {
                                    ...
                                    console.error(`MapBox API Error for ${type} segment ${i}:`, data.message);
                                }
                            } catch (error) {
