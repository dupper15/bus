...
const TaskPage = () => {
  ...
  const findMatchingLine = (taskLine) => {
    if (!lines.length || !taskLine) return null;
    return (
        lines.find(
            (line) => line.name.toLowerCase() === taskLine.name.toLowerCase()
        ) ||
        lines.find((line) =>
            line.name.toLowerCase().includes(taskLine.name.toLowerCase())
        ) ||
        null
    );
  };
  ...
  const handleLineClick = (taskLine) => {
    const matchedLine = findMatchingLine(taskLine);
          ...
          <div className="overflow-x-auto rounded-lg bg-white shadow-md">
            <Table className="overflow-hidden rounded-t-lg border border-gray-300">
              <TableHeader className="bg-green-500 rounded-t-lg pointer-events-none">
                <TableRow>
                  {["Name", "Start Station", "License Plate", "Time Start", "Status"].map(
                      (header, idx) => (
                          <TableHead
                              key={idx}
                ...
                >
                  ...
                  <span className="font-semibold text-gray-700">
                    ...
                    <div className="text-sm flex flex-col text-gray-600">
                      <span className="font-semibold text-gray-700">Time start:</span>
                      <div>{item.time}</div>
                    </div>
                    <div
