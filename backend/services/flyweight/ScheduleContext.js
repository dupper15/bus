...
class ScheduleContext {
	...
	async loadFlyweights(repeatingIds) {
		this.flyweights.bus = await flyweightFactory.getFlyweight(
			"bus",
			repeatingIds.busId
		);
		this.flyweights.line = await flyweightFactory.getFlyweight(
			"line",
			repeatingIds.lineId
		);
		this.flyweights.driver = await flyweightFactory.getFlyweight(
			"employee",
			repeatingIds.driverId
		);
		this.flyweights.busboy = await flyweightFactory.getFlyweight(
			"employee",
			repeatingIds.busboyId
		);
	}
